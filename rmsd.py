...
def compare_csv(file):
    ...
    merged_data = pd.merge(data_reference, data_to_compare, on=['gene', 'perturbation'], suffixes=('_ref', '_comp'))
    ...
    axes[1, 1].plot(sorted_yours, np.linspace(0, 1, len(sorted_yours)), label='Yours')
    axes[1, 1].plot(sorted_other, np.linspace(0, 1, len(sorted_other)), label='Other')
